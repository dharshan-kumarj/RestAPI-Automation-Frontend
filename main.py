...
async def register_user():
    print("Request received at /register")
    try:
...
async def verify_user():
    ...
    return jsonify({"valid": False, "message": "Wrong OTP. Please try again."})
...
async def login_user():
    ...
    if result["valid"]:
        ...
        return jsonify({"valid": True, "message": "Login successful", "token": token["token"]})
        ...
        return jsonify(
            {
                ...
                "username": await user.get("username"), # Add this line
                "client_ip": client_ip,
            }
        )
